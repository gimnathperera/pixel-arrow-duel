import type { GameState, PlayerState, ArrowState, Platform, ParticleState, Rectangle } from './types';
import { INITIAL_PLATFORMS, GAME_WIDTH, GAME_HEIGHT } from '../game/MapDef';
...
export class GameEngine {
  ...
  private getInitialState(): GameState {
    return {
      players: [
        { id: 'p1', x: 200, y: 300, width: 24, height: 24, color: '#ef4444', velocity: { x: 0, y: 0 }, health: MAX_HEALTH, isGrounded: false, facingRight: true, shootCooldown: 0 },
        { id: 'p2', x: 600, y: 300, width: 24, height: 24, color: '#3b82f6', velocity: { x: 0, y: 0 }, health: MAX_HEALTH, isGrounded: false, facingRight: false, shootCooldown: 0 }
      ],
  ...
  private updatePlatforms() {
    for (const plat of this.state.platforms) {
      if (plat.type === 'moving' && plat.baseX !== undefined && plat.range !== undefined && plat.dir !== undefined && plat.speed !== undefined) {
  ...
  private updatePlayers() {
    ...
    // Input Handling
    // Player 1: WASD + Space
    if (this.keys.has('KeyA')) { p1.velocity.x = -MOVE_SPEED; p1.facingRight = false; }
    else if (this.keys.has('KeyD')) { p1.velocity.x = MOVE_SPEED; p1.facingRight = true; }
    else { p1.velocity.x = 0; }

    if (this.keys.has('KeyW') && p1.isGrounded) { p1.velocity.y = JUMP_FORCE; p1.isGrounded = false; }
    ...
    if (this.keys.has('ArrowLeft')) { p2.velocity.x = -MOVE_SPEED; p2.facingRight = false; }
    else if (this.keys.has('ArrowRight')) { p2.velocity.x = MOVE_SPEED; p2.facingRight = true; }
    else { p2.velocity.x = 0; }

    if (this.keys.has('ArrowUp') && p2.isGrounded) { p2.velocity.y = JUMP_FORCE; p2.isGrounded = false; }
            ...
            if (player.velocity.y > 0) {
                ...
                if (player.y + player.height - player.velocity.y <= plat.y) {
                    ...
                    if (plat.type === 'jump-bar') {
                        ...
                        this.spawnParticles(player.x + player.width/2, player.y + player.height, '#fbbf24', 5);
                    } else {
                        player.velocity.y = 0;
                        if (plat.type === 'moving' && plat.dir !== undefined && plat.speed !== undefined) {
  ...
  private takeDamage(playerId: string, amount: number) {
    ...
    this.spawnParticles(player.x + player.width/2, player.y + player.height/2, player.color, 15);

    if (player.health <= 0) {
      player.health = 0;
      this.state.matchOver = true;
      this.state.winner = playerId === 'p1' ? 'Player 2 (Blue)' : 'Player 1 (Red)';
    ...
    for (const arrow of this.state.arrows) {
      ...
      for (const plat of this.state.platforms) {
        if (plat.type !== 'jump-bar' && this.checkAABB(arrow, plat)) {
          arrow.active = false;
          hitPlatform = true;
          this.spawnParticles(arrow.velocity.x > 0 ? arrow.x+arrow.width : arrow.x, arrow.y, '#9ca3af', 5);
          break;
        }
      }
  ...
  private draw() {
    ...
    for (const plat of this.state.platforms) {
      if (plat.type === 'jump-bar') {
        ...
        this.ctx.strokeRect(plat.x + 1, plat.y + 1, plat.width - 2, plat.height - 2);
